...
export function getMeta() {
  ...
  const publicCount = channels.filter((c) => !isPrivateChannel(c.name)).length;
  return {
    name: d.name || "Toffee",
...
export function getExpiryMs() {
  const channels = getChannels();
  let minExpiryMs = null;
  for (const ch of channels) {
    if (ch?.headers?.cookie) {
      const m = ch.headers.cookie.match(/Expires=(\d+)/);
      if (m) {
        const ms = Number(m[1]) * 1000;
        if (minExpiryMs === null || ms < minExpiryMs) {
          minExpiryMs = ms;
        }
      }
    }
...
export function getCategories() {
  ...
  for (const c of getChannels()) {
    if (isPrivateChannel(c.name)) continue;
...
/** Check if a channel should be hidden from the homepage and public lists. */
export function isPrivateChannel(name) {
  ...
  return (
    ...
    n === "ban vs aus" ||
    n === "wc match recap" ||
    n.startsWith("fifa wc channel") ||
    n.startsWith("bfl live")
  );
}
